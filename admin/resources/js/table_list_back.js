...
var tFlexs = {
    ...
    init: (obj) => {
        obj.totalSize = tFlexs.ckbSize; 
        $('#'+ obj.id +' .motiv_tbl .hd_item[keyname=checkedbox]').css('width', '40px');

        $.each( obj.tableSize, (name, size) => {
            $('#'+ obj.id +' .motiv_tbl .hd_item[keyname='+ name +']').css({
                'width': size +'px',
                'left': obj.totalSize +'px'
            });
...
var tFlexLocks = {
    ...
    init: (obj) => {
        obj.totalSize = tFlexNomal.ckbSize; 
        $('#'+ obj.id +' .motiv_tbl .hd_item[keyname=checkedbox]').css('width', '40px');
        ...
        obj.scrollLookSize = tFlexNomal.ckbSize;

        $.each( obj.scrollLook, (name, size) => {
            obj.scrollLookSize = obj.scrollLookSize + obj.tableSize[size];
        });

        console.log(obj.totalSize)
        
        $('#'+ obj.id +' .motiv_tbl').find('.tbl_head, .tbl_body').css('width', obj.totalSize +'px');
        $('#'+ obj.id +' .motiv_tbl').find('.look_head_head, .look_body_head').css('width', obj.scrollLookSize +'px');
        $('#'+ obj.id +' .motiv_tbl').find('.look_head_list, .look_body_box').css('width', (obj.totalSize-obj.scrollLookSize) +'px');
        
        $('#'+ obj.id +' .motiv_tbl').find('.row_item').css('width', (obj.totalSize-290) +'px');
        console.log()
    },
...
var tFlex = {
    ...
    init: (id, obj, test) => {
        ...
        if (obj.scrollLookSta) {// tableData.scrollLookSta의 값이 true 일때 실행됨니다.
            ...
            $('#'+id+' .motiv_tbl').addClass('lock_tbl');
        } else {
...
var tableFlex = {
    ...
    lookWheelUi: (id) => {
        var wheelStart = 0;
        $('#'+id+' .motiv_tbl.lock_tbl .tbl_inner .tbl_body .look_body_box .look_body_list').on("mousewheel", function (e) {
            var wheel = e.originalEvent.deltaX;
            wheelStart = wheelStart - wheel;
